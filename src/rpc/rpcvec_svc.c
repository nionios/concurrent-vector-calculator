...
main (int argc, char **argv)
{
	...
	pmap_unset (VEC_PROGRAM, VEC_VERS);
	...
	if (!svc_register(transp, VEC_PROGRAM, VEC_VERS, vec_program_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (VEC_PROGRAM, VEC_VERS, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, VEC_PROGRAM, VEC_VERS, vec_program_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (VEC_PROGRAM, VEC_VERS, tcp).");
		exit(1);
	}
